...
- \pagestyle{fancy}
- \fancyhf{}
- \fancyhead[C]{Meng Yang\ 640815\ |\ Glib\ 2021123457}
- \fancyfoot[C]{\thepage}
...
knitr::opts_chunk$set(
  ...
  fig.height = 3,
  fig.align  = "center",
  dev.args   = list(pointsize = 8),   # 图内文字 10pt
...
The dataset contains weekly sales data for Coca-Cola and PepsiCo products across various markets. 
```{r variable-table, results='asis'}
...
ggplot(plot_dat,
       aes(x = factor(YEAR),
           y = value,
           fill = Brand)) +
  geom_col(position = "dodge", width = 0.5, alpha = 0.9)+ # adjust bar
  geom_text(aes(label = scales::comma(value, accuracy = 1),
                group = Brand),
            position = position_dodge(.5),
            vjust = 0.4, size = 2.2) + #adjust bar number font
  ...
  theme(
    ...
    axis.text.y     = element_text(size = 6),   # Y font
...
ggplot(brand_variety, aes(x = "", y = pct, fill = reorder(variety, pct))) +
  ...
  coord_polar(theta = "y") +
  geom_text(aes(label = percent(pct, accuracy = 0.1), x = 1.25),
  ...
  theme(
    ...
    plot.title = element_text(size = 10, face = "bold",
                              margin = margin(b = 6)),  # margin between title and pie
    panel.spacing = unit(10, "pt"),     # horitontal space between pie
    strip.text  = element_text(size = 10),
    plot.margin = margin(2, 2, 2, 2, "pt")  # squeeze the space- margin
  )
...
````{r package preference with avg price}
